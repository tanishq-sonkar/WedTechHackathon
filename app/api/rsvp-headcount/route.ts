  ...
  try {
    ...
    const eventMap: Record<
      string,
      {
        ...
        maybe: number;
        total_heads: number;
        dietary: { veg: number; non_veg: number; jain: number; vegan: number };
      }
    > = {};
    ...
    for (const row of (data ?? []) as RsvpRow[]) {
      ...
      if (!eventMap[evt.id]) {
        eventMap[evt.id] = {
          ...
          maybe: 0,
          total_heads: 0,
          dietary: { veg: 0, non_veg: 0, jain: 0, vegan: 0 },
        };
      }

      const bucket = eventMap[evt.id];
      const heads = 1 + (row.plus_ones_count ?? 0) + (row.children_count ?? 0);

      if (row.status === 'attending') {
        bucket.attending += 1;
        bucket.total_heads += heads;
      } else if (row.status === 'not_attending') {
        bucket.not_attending += 1;
      } else if (row.status === 'maybe') {
        bucket.maybe += 1;
      }

      if (row.dietary_pref) {
        bucket.dietary[row.dietary_pref] += 1;
      }
    }
